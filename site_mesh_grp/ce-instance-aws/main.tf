...
resource "volterra_securemesh_site_v2" "site" {
  ...
  name = format("%s-%s-%02d", data.terraform_remote_state.aws_infra.outputs.f5xc_ce_site_name,
  data.terraform_remote_state.aws_infra.outputs.random_suffix, count.index + 1)
  namespace               = "system"
  ...
  aws {
    not_managed {
      node_list {
        public_ip = data.terraform_remote_state.azure_infra.outputs.public_ip
        interface_list {
          network_option {
