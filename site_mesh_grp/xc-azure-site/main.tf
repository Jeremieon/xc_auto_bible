...
resource "volterra_securemesh_site_v2" "site" {
  depends_on              = [volterra_known_label_key.vsite_key, volterra_known_label.vsite_label, azurerm_public_ip.ce_public_ip, var.aws_public_ip]
  ...
  azure {
    not_managed {
      node_list {
        public_ip = data.terraform_remote_state.aws_infra.outputs.public_ip
        interface_list {
          network_option {
